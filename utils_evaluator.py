...
def evaluate_square(cropped_to_bound: np.ndarray, x_index: int, svm_classifier, knn_classifier, alexnet) -> int:
    ...
    if svm_classifier is not None and knn_classifier is not None and alexnet is not None:
        alexnet_pred = alexnet.predict(cropped_to_bound.reshape(-1, Globals.CLASSIFIER_IMG_DIM,
                                                                       Globals.CLASSIFIER_IMG_DIM, 1), verbose=0)
        if x_index % 7 == 0:
            ...
            alexnet_pred = alexnet_pred[3:]
            alexnet_pred_shift = 3
        else:
            ...
            alexnet_pred = alexnet_pred[:3]
            alexnet_pred_shift = 0
        alexnet_pred = np.argmax(alexnet_pred) + alexnet_pred_shift
        if svm_pred == knn_pred == alexnet_pred:
            return knn_pred  # if they agree, return one of them
        else:
            # if they disagree, choose the most voted option
            chosen_pred = Counter([svm_pred, knn_pred, alexnet_pred]).most_common(1)[0][0]
            return chosen_pred
    else:
        return 0